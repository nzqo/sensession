...
def set_seeds(seed_value: int) -> None:
    """
    Set seeds (use repo-provided part to match reference as closely as possible)
    """
    trainer.setup_seed(seed_value)
...
def build_loader(
    ...
    """
    Build a DataLoader for our dataset.

    ReConFormer training expects input shape (N, T, C) and authors permute
    in the trainer themselves.
    """
    ...
    if feature_array_copy.shape[2] != 56:
...
def evaluate(
    ...
    with torch.no_grad():
        for batch_inputs, batch_labels in tensor_loader:
            ...
            # ReConFormer expects [batch, C, T]; dataloader gives [batch, T, C]
            # trainer performs this permutation internally as well.
            batch_inputs = torch.permute(batch_inputs, (0, 2, 1))
...
# ----------------------------------------------------------------------
# Internal helper to train ReConFormer once
# ----------------------------------------------------------------------
def _train_reconformer_once(
    train_loader: DataLoader,
    seed: int,
    model_args: dict,
) -> torch.nn.Module:
    """
    Train ReConFormer using its original training pipeline and return the trained model.
    """
    set_seeds(seed)

    args = dict(model_args)

    model, best_val_acc, _ = trainer.train_once(
        args=args,
    ...
    print(f"  Best train/val accuracy during training: {best_val_acc:.4f}")
    return model
...
def _run_cross_receiver_collect_records(
    ...
    """
    print(
        f"Starting cross-receiver experiment with ReConFormer - Train on one, test on others.\n"
    ...
    batch_size_train = int(model_args["batch_size"])
    batch_size_eval = batch_size_train

    receiver_names = get_receiver_names(meta_df)
    ...
    for receiver_index, train_receiver in enumerate(receiver_names):
        receiver_start_time = time.time()
        print(
            f"Training ReConFormer on receiver {train_receiver} "
            f"({receiver_index + 1}/{total_receivers})"
        )

        meta_train_df = meta_df.filter(pl.col("receiver_name") == train_receiver)
        ...
        for eval_receiver in receiver_names:
            if eval_receiver == train_receiver:
                continue

            meta_eval_df = meta_df.filter(pl.col("receiver_name") == eval_receiver)
            ...
            print(
                f"Trained on {train_receiver} "
                f"tested on {eval_receiver} "
    ...
    print(
        f"\nReConFormer cross-receiver summary: \n"
        f"  Dataset.........: {dataset_name}\n"
        f"  Standardization.: {standardization.name}\n"
...
def run_experiment_cross_receiver(
    ...
    if write_csv:
        ...
        print(f"Saved ReConFormer cross receiver results to {output_path}")
...
def run_std_ablation_cross_receiver(
    ...
    print(f"Saved ReConFormer cross standardization ablation to {output_path}")
    return output_path
...
# K-fold experiment
# ----------------------------------------------------------------------
def run_experiment_kfold_reconformer(
    dataset_name: str,
    csi_frame: pl.DataFrame,
    ...
    """
    print(
        f"Running k-fold ReConFormer experiment to estimate model accuracies per receiver:\n"
    ...
    for fold_index, (train_indices, test_indices) in enumerate(
        ...
        print(
            f"ReConFormer fold summary: [on {dataset_name}]\n"
...
def get_patched_config(meta_df: pl.DataFrame) -> dict:
    """
    We use the UT-HAR config and just patch to match our shapes.
    """
    config = load_json("configs/UT-HAR.json")
    num_classes = int(meta_df.get_column("activity_idx").to_numpy().max()) + 1
    config["data_shape"] = [56, 700]
...
def main() -> None:
    """
    Entry point for running ReConFormer experiments from the command line.
    ...
    if parsed_arguments.dataset and parsed_arguments.mode:
        ...
        csi_df, meta_df = load_parquet_dataset(dataset_path)
        model_args = get_patched_config(meta_df)
        ...
        if parsed_arguments.mode == "kfold":
            run_experiment_kfold_reconformer(
                dataset_name=dataset_name,
                csi_frame=csi_df,
                meta_frame=meta_df,
                standardization=standardization,
